...
import { StarIcon, LockClosedIcon, FireIcon } from '@heroicons/react/24/solid'
...
const AdminPanel = () => {
  ...
  // Estado para Marketing Hero
  const [marketingHeroForm, setMarketingHeroForm] = useState({
    backgroundImage: '',
    title: 'Não perca sua chance!',
    subtitle: 'Turma fechando em breve',
    urgencyText: 'Últimas vagas disponíveis',
    motivationalTexts: ['Seu futuro começa aqui', 'Transforme sua carreira hoje', 'Aprovação está mais perto do que você imagina'],
    ...
    active: true,
    order: 1,
    backgroundAnimationType: 'sparks', // sparks, fire, stars, none
    backgroundAnimationActive: true
  })
  const [marketingHeroImage, setMarketingHeroImage] = useState(null)
  const [savingMarketingHero, setSavingMarketingHero] = useState(false)
...
- Baseie-se EXCLUSIVAMENTE no conteúdo fornecido acima`
  ...
  // Carregar configuração do Marketing Hero
  useEffect(() => {
    if (!isAdmin) return
    
    const marketingRef = collection(db, 'marketingHero')
    const q = query(marketingRef, where('active', '==', true), orderBy('order', 'asc'), limit(1))
    
    const unsub = onSnapshot(q, (snapshot) => {
      if (!snapshot.empty) {
        const data = snapshot.docs[0].data()
        setMarketingHeroForm({
          ...
          motivationalTexts: data.motivationalTexts || ['Seu futuro começa aqui', 'Transforme sua carreira hoje', 'Aprovação está mais perto do que você imagina'],
      ...
      console.error('Erro ao carregar Marketing Hero:', error)
    })
    
    return () => unsub()
  }, [isAdmin])

  // Upload de imagem do Marketing Hero
  const handleMarketingHeroImageUpload = (event) => {
    const file = event.target.files?.[0]
    if (!file) return
    ...
    if (file.size > 2 * 1024 * 1024) {
      setMessage('❌ A imagem é muito grande. Máximo: 2MB. Use imagens menores ou comprima antes de enviar.')
      return
    }

    const reader = new FileReader()
    reader.onload = (e) => {
      setMarketingHeroImage(e.target.result)
      setMarketingHeroForm(prev => ({
        ...prev,
  ...
  // Salvar configuração do Marketing Hero
  const saveMarketingHero = async () => {
    if (!isAdmin) {
      setMessage('❌ Apenas administradores podem salvar configurações.')
      return
    }

    setSavingMarketingHero(true)
    try {
      // Preparar dados sem o id
      const { id, ...formDataWithoutId } = marketingHeroForm
      
      const configData = {
        ...formDataWithoutId,
        backgroundImage: marketingHeroImage || marketingHeroForm.backgroundImage,
        timerEndDate: marketingHeroForm.showTimer && marketingHeroForm.timerEndDate 
          ? new Date(marketingHeroForm.timerEndDate).toISOString() 
          : null,
        updatedAt: serverTimestamp(),
      }

      // Se já existe, atualizar; senão, criar novo
      if (marketingHeroForm.id) {
        await updateDoc(doc(db, 'marketingHero', marketingHeroForm.id), configData)
        setMessage('✅ Marketing Hero atualizado com sucesso!')
      } else {
        await addDoc(collection(db, 'marketingHero'), {
          ...configData,
          createdAt: serverTimestamp(),
        })
        setMessage('✅ Marketing Hero criado com sucesso!')
      }
    } catch (error) {
      console.error('Erro ao salvar Marketing Hero:', error)
      setMessage('❌ Erro ao salvar configuração. Tente novamente.')
    } finally {
      setSavingMarketingHero(false)
    }
  }

  // Deletar Marketing Hero
  const deleteMarketingHero = async () => {
    if (!isAdmin) {
      setMessage('❌ Apenas administradores podem deletar configurações.')
      return
    }

    if (!marketingHeroForm.id) {
      setMessage('❌ Não há configuração para deletar.')
      return
    }

    if (!confirm('Tem certeza que deseja deletar o Marketing Hero? Esta ação não pode ser desfeita.')) {
      return
    }

    try {
      await deleteDoc(doc(db, 'marketingHero', marketingHeroForm.id))
      setMessage('✅ Marketing Hero deletado com sucesso!')
      // Resetar formulário
      setMarketingHeroForm({
        backgroundImage: '',
        title: 'Não perca sua chance!',
      ...
      setMarketingHeroImage(null)
    } catch (error) {
      console.error('Erro ao deletar Marketing Hero:', error)
      setMessage('❌ Erro ao deletar configuração. Tente novamente.')
    }
  }
...
- Apenas o objeto JSON puro começando com { e terminando com }`
  ...
  const tabs = [
    ...
    { id: 'marketing', label: '🔥 Marketing Hero', icon: '🔥' },
                ...
                <div className="relative overflow-hidden bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700 p-6">
                  ...
                  <div className="relative">
                    <p className="flex items-center gap-2 text-sm font-semibold text-alego-600 mb-4">
                      ...
                      Gerenciar Marketing Hero (Seção Persuasiva)
                    </p>
                    <p className="text-xs text-slate-500 mb-6">
                      Configure a seção final da página inicial com imagem de fundo, textos persuasivos e elementos de urgência para aumentar conversões.
                    </p>
                    ...
                    <div className="space-y-6">
                      ...
                      <div>
                        <label className="block text-xs font-semibold text-slate-600 mb-2">
                          Imagem de Fundo (Capa) - Máximo 2MB
                        </label>
                        <input
                          type="file"
                          accept="image/*"
                          onChange={handleMarketingHeroImageUpload}
                          className="w-full rounded-lg border border-slate-300 p-2 text-sm"
                        />
                        {(marketingHeroForm.backgroundImage || marketingHeroImage) && (
                          <div className="mt-2">
                            <img
                              src={marketingHeroImage || marketingHeroForm.backgroundImage}
                              alt="Preview"
                        ...
                        <p className="text-xs text-slate-500 mt-1">
                          Esta imagem aparecerá como fundo da seção com efeito de fagulhas de fogo.
                        </p>
                      </div>
                      ...
                      <div>
                        ...
                        <input
                          type="text"
                          value={marketingHeroForm.title}
                          onChange={(e) => setMarketingHeroForm(prev => ({ ...prev, title: e.target.value }))}
                          placeholder="Ex: Não perca sua chance!"
                          className="w-full rounded-lg border border-slate-300 p-2 text-sm"
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <input
                          type="text"
                          value={marketingHeroForm.subtitle}
                          onChange={(e) => setMarketingHeroForm(prev => ({ ...prev, subtitle: e.target.value }))}
                          placeholder="Ex: Turma fechando em breve"
                          className="w-full rounded-lg border border-slate-300 p-2 text-sm"
                        />
                      </div>
                      ...
                      <div>
                        <label className="block text-xs font-semibold text-slate-600 mb-2">
                          Texto de Urgência (Badge vermelho)
                        </label>
                        <input
                          type="text"
                          value={marketingHeroForm.urgencyText}
                          onChange={(e) => setMarketingHeroForm(prev => ({ ...prev, urgencyText: e.target.value }))}
                          placeholder="Ex: Últimas vagas disponíveis"
                          className="w-full rounded-lg border border-slate-300 p-2 text-sm"
                        />
                      </div>
                      ...
                      <div>
                        <label className="block text-xs font-semibold text-slate-600 mb-2">
                          Textos Motivacionais (um por linha, rotacionam automaticamente)
                        </label>
                        <textarea
                          value={marketingHeroForm.motivationalTexts?.join('\n') || ''}
                          onChange={(e) => {
                            const texts = e.target.value.split('\n').filter(t => t.trim())
                            setMarketingHeroForm(prev => ({ ...prev, motivationalTexts: texts }))
                          }}
                          placeholder="Seu futuro começa aqui&#10;Transforme sua carreira hoje&#10;Aprovação está mais perto do que você imagina"
                          rows={4}
                          className="w-full rounded-lg border border-slate-300 p-2 text-sm"
                        />
                        <p className="text-xs text-slate-500 mt-1">
                          Cada linha será um texto que aparece rotativamente na seção.
                        </p>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <input
                            type="text"
                            value={marketingHeroForm.ctaText}
                            onChange={(e) => setMarketingHeroForm(prev => ({ ...prev, ctaText: e.target.value }))}
                      ...
                      <div className="border-t border-slate-200 dark:border-slate-700 pt-4">
                        <div className="flex items-center gap-2 mb-4">
                          <input
                            type="checkbox"
                            checked={marketingHeroForm.showTimer}
                            onChange={(e) => setMarketingHeroForm(prev => ({ ...prev, showTimer: e.target.checked }))}
                            className="rounded"
                          />
                          <label className="text-xs font-semibold text-slate-600">Mostrar Timer de Contagem Regressiva</label>
                        </div>
                        {marketingHeroForm.showTimer && (
                          <div>
                            <label className="block text-xs font-semibold text-slate-600 mb-2">
                              Data/Hora Final do Timer
                            </label>
                            <input
                              type="datetime-local"
                              value={marketingHeroForm.timerEndDate ? new Date(marketingHeroForm.timerEndDate).toISOString().slice(0, 16) : ''}
                              onChange={(e) => setMarketingHeroForm(prev => ({ ...prev, timerEndDate: e.target.value }))}
                              className="w-full rounded-lg border border-slate-300 p-2 text-sm"
                            />
                          </div>
                        )}
                      ...
                      <div className="border-t border-slate-200 dark:border-slate-700 pt-4">
                        <div className="flex items-center gap-2 mb-4">
                          <input
                            type="checkbox"
                            checked={marketingHeroForm.showSpotsLeft}
                            onChange={(e) => setMarketingHeroForm(prev => ({ ...prev, showSpotsLeft: e.target.checked }))}
                            className="rounded"
                          />
                          <label className="text-xs font-semibold text-slate-600">Mostrar Vagas Restantes</label>
                        </div>
                        {marketingHeroForm.showSpotsLeft && (
                          <div>
                            <label className="block text-xs font-semibold text-slate-600 mb-2">
                            ...
                            <input
                              type="number"
                              value={marketingHeroForm.spotsLeft}
                              onChange={(e) => setMarketingHeroForm(prev => ({ ...prev, spotsLeft: parseInt(e.target.value) || 0 }))}
                              min="0"
                              className="w-full rounded-lg border border-slate-300 p-2 text-sm"
                            />
                          </div>
                      ...
                      <div className="border-t border-slate-200 dark:border-slate-700 pt-4">
                        <div className="flex items-center gap-2 mb-4">
                          <input
                            type="checkbox"
                            checked={marketingHeroForm.backgroundAnimationActive}
                            onChange={(e) => setMarketingHeroForm(prev => ({ ...prev, backgroundAnimationActive: e.target.checked }))}
                            className="rounded"
                          />
                          <label className="text-xs font-semibold text-slate-600">Animação de Fundo Ativa (em toda a página)</label>
                        </div>
                        {marketingHeroForm.backgroundAnimationActive && (
                          <div>
                            <label className="block text-xs font-semibold text-slate-600 mb-2">
                              Tipo de Animação
                            </label>
                            <select
                              value={marketingHeroForm.backgroundAnimationType}
                              onChange={(e) => setMarketingHeroForm(prev => ({ ...prev, backgroundAnimationType: e.target.value }))}
                              className="w-full rounded-lg border border-slate-300 p-2 text-sm"
                            >
                              <option value="sparks">Fagulhas (Laranja/Amarelo)</option>
                              <option value="fire">Fogo (Vermelho/Laranja)</option>
                              <option value="stars">Estrelas/Faíscas (Branco/Azul)</option>
                              <option value="none">Nenhuma</option>
                            </select>
                            <p className="text-xs text-slate-500 mt-1">
                              A animação aparecerá em toda a página, não apenas no card do Marketing Hero.
                            </p>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        <input
                          type="checkbox"
                          checked={marketingHeroForm.active}
                          onChange={(e) => setMarketingHeroForm(prev => ({ ...prev, active: e.target.checked }))}
                          className="rounded"
                        />
                        <label className="text-xs font-semibold text-slate-600">Marketing Hero Ativo</label>
                      </div>
                      ...
                      <div className="flex gap-3">
                        <button
                          type="button"
                          onClick={saveMarketingHero}
                          disabled={savingMarketingHero}
                          className="flex-1 rounded-xl bg-gradient-to-r from-orange-500 to-red-500 px-6 py-3 text-sm font-bold text-white shadow-lg hover:shadow-xl transition-all disabled:opacity-50"
                        >
                          {savingMarketingHero ? 'Salvando...' : '💾 Salvar Configuração'}
                        </button>
                        {marketingHeroForm.id && (
                          <button
                            type="button"
                            onClick={deleteMarketingHero}
                            disabled={savingMarketingHero}
                            className="rounded-xl bg-rose-600 px-6 py-3 text-sm font-bold text-white shadow-lg hover:shadow-xl transition-all disabled:opacity-50 flex items-center gap-2"
                          >
                      ...
                      {marketingHeroForm.id && (
                        <div className="rounded-lg bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 p-3">
                          <p className="text-xs text-blue-700 dark:text-blue-300">
                            <strong>Configuração existente:</strong> Você está editando uma configuração salva. 
                            Clique em "Salvar Configuração" para atualizar ou "Excluir" para remover.
                          </p>
                        </div>
                      )}
                    </div>
