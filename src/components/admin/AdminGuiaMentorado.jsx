...
export default function AdminGuiaMentorado() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setFeedback(
        `✅ Configuração salva no Firestore (admin online). Liberação diária às ${formatDailyReleaseLabel(saved)} (Brasília).${
          saved.enabled
  ...
  const handleToggleAutomation = async () => {
    ...
    try {
      ...
      setFeedback(
        next
          ? `✅ Automação ligada no Firestore (admin online) para “${courseName}”. O cron horário gera no horário ${releaseLabel} (Brasília), se o gatilho “Tick admin online” estiver ativo.`
          : `⏸️ Automação desligada no Firestore (admin online) para “${courseName}”. O cron não processa este curso.`,
      )
    } catch (err) {
      patchForm({ enabled: !next })
    ...
    try {
      ...
      const errText =
        result.errors.length > 0
          ...
          : ''
      setFeedback(
        `✅ Configuração aplicada a ${result.count}/${result.total} curso(s) no Firestore (admin online).${errText}`,
      )
    } catch (err) {
      ...
      <div className="cp-card !rounded-2xl p-5">
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-[1fr_auto] sm:items-end">
          ...
          <button
            ...
            title="Grava imediatamente no Firestore (admin online) (Firestore)"
          >
            {saving && !busyAction
              ? 'Salvando…'
              : form.enabled
            ...
            <div className="flex items-start gap-2">
              ...
              <div>
                ...
                <p className="mt-1 text-[11px] text-cp-muted">
                  Com automação ligada + “Tick admin online” + Salvar/Aplicar, o servidor
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <div className="cp-card !rounded-2xl space-y-4 p-5">
          ...
          <div className="space-y-2 rounded-xl border border-cp-border bg-cp-surface/40 p-3">
            ...
            {[
              ['onDailyCron', 'Tick admin online (no horário)'],
