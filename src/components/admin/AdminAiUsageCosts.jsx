...
import { getAdminAiBudgetStatus } from '../../services/adminAiBudget'
...
export default function AdminAiUsageCosts() {
  ...
  const [budget, setBudget] = useState(() => getAdminAiBudgetStatus())
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setEvents(ev)
      setDaily(dy)
      setBudget(getAdminAiBudgetStatus())
    } catch (err) {
  ...
  const budgetPct = budget.limit ? Math.min(100, Math.round((budget.used / budget.limit) * 100)) : 0
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wide text-amber-700 dark:text-amber-300">
              Orçamento automação (jobs caros / hora)
            </p>
            <p className="mt-1 text-lg font-black text-slate-900 dark:text-white">
              {budget.used}/{budget.limit} usados
              <span className="ml-2 text-sm font-semibold text-slate-500">
                · {budget.remaining} restantes
              </span>
            </p>
            <p className="mt-1 text-xs text-slate-600 dark:text-slate-400">
              Limite local da fila automática — cliques manuais do admin não são bloqueados.
            </p>
          </div>
          <div className="h-2 w-40 overflow-hidden rounded-full bg-amber-200/80 dark:bg-amber-950">
            <div
              className={`h-full rounded-full ${budgetPct >= 90 ? 'bg-red-500' : 'bg-amber-500'}`}
