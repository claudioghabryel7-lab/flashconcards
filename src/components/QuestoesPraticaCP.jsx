...
import { alternativasAsOrderedObject, sortAlternativasEntries } from '../utils/questaoAlternativas'
...
function normalizeAlternativas(questao) {
  return alternativasAsOrderedObject(questao?.alternativas)
}
...
export function QuestaoAlternativas({
  ...
  return (
    <div className="space-y-2.5">
      {sortAlternativasEntries(normalizeAlternativas(questao)).map(([key, value]) => {
        const isCorrect = key === correta
        const isSelected = selectedAnswer === key
        const idle = !reveal
        return (
