...
async function mountExportClone(source) {
  const wrapper = document.createElement('div')
  wrapper.className = 'material-pdf-capture-root material-pdf-export'
...
async function captureBlocksToPdf(blocks, filename) {
  ...
  for (const block of blocks) {
    ...
    if (block.offsetHeight < 2 && block.textContent?.trim() === '') continue

    const canvas = await toCanvas(block, {
      pixelRatio: CAPTURE_PIXEL_RATIO,
      backgroundColor: '#ffffff',
      cacheBust: true,
      width: A4_CONTENT_WIDTH_PX - 64,
    })
...
/** Captura o HTML visível na tela — fiel ao layout do site. */
export async function downloadElementAsPdf(element, filename = 'material.pdf') {
  ...
  const { wrapper, cleanup } = await mountExportClone(element)

  try {
    const blocks =
      wrapper.children.length > 0 ? Array.from(wrapper.children) : [wrapper]
    await captureBlocksToPdf(blocks, filename)
  } finally {
    cleanup()
  }
}
...
export async function downloadMaterialPdf(material, filename = 'material.pdf', options = {}) {
  ...
  try {
    ...
    const blocks =
      body.children.length > 0 ? Array.from(body.children) : [body]
    await captureBlocksToPdf(blocks, filename)
  } finally {
...
<body class="material-pdf-export material-pdf-capture-root">
  ${clone.outerHTML}
</body>
</html>`
...
/** Abre diálogo de impressão (Destino: Salvar como PDF) — máxima fidelidade. */
export function printMaterial(material, options = {}) {
  if (!material && !options.element) {
    throw new Error('Nenhum material para imprimir.')
  }

  if (options.element) {
    printElement(options.element)
    return
  }
...
export function printConcursoMaterial(material, options = {}) {
  return printMaterial(material, options)
}

/** @deprecated use downloadMaterialPdf */
export async function downloadConcursoMaterialPdf(material, filename = 'material.pdf', options = {}) {
  return downloadMaterialPdf(material, filename, options)
}

// --- HTML estático (fallback quando não há elemento DOM) ---
...
const PRINT_STYLES = `
  ...
  * { box-sizing: border-box; }
  ...
  .card-orange { background: linear-gradient(135deg, #fff7ed 0%, #fef3c7 100%); border: 1px solid #fed7aa; }
  .card-blue { background: linear-gradient(135deg, #eff6ff 0%, #e0e7ff 100%); border: 1px solid #bfdbfe; }
  .card-red { background: #fff1f2; border: 1px solid #fecdd3; }
  .card-neutral { background: #f8fafc; border: 1px solid #e2e8f0; }
  .card-section {
    background: #f8fafc;
  ...
  .sub-title { font-size: 10.5pt; font-weight: 700; color: #475569; margin: 0 0 8px; }
  .hot-list { margin: 0; padding: 0; list-style: none; }
  .hot-list li { display: flex; gap: 8px; margin-bottom: 6px; font-size: 10.5pt; color: #475569; }
  ...
  .questao-badge { font-size: 9pt; font-weight: 700; color: #7c3aed; margin-bottom: 8px; display: block; }
  .questao-enunciado { font-size: 10.5pt; color: #334155; margin: 0 0 12px; white-space: pre-line; }
  .alt {
    border-radius: 8px;
    padding: 10px 12px;
    margin-bottom: 8px;
  ...
  .alt.correct { background: #dcfce7; border: 2px solid #22c55e; color: #166534; font-weight: 600; }
  .gabarito-box { background: #eff6ff; border-radius: 8px; padding: 12px; margin-top: 10px; }
  .gabarito-box h5 { margin: 0 0 6px; font-size: 10pt; color: #1d4ed8; }
  .tipo-badge {
  ...
  .ia-content-enhanced table { width: 100%; border-collapse: collapse; margin: 14px 0; border: 1px solid #cbd5e1; }
...
/** HTML de impressão (fallback sem DOM). */
