  ...
  cors(req, res, async () => {
    ...
    try {
      ...
      const emailPromises = usersSnapshot.docs.map(async (userDoc) => {
        ...
        if (!userEmail) {
          errorCount++
          errors.push(`Usuário ${userDoc.id} não tem email cadastrado`)
          return
        }
...
${message}
                    ...
                    <div class="button-container">
                      <a href="https://www.flashconcards.com.br" class="button">Acessar Plataforma</a>
                    </div>
                  </div>
                  <div class="footer">
                    <p><strong>Equipe Plegimentoria ALEGO</strong></p>
                    <p>Este é um email automático, por favor não responda.</p>
                    ${adminEmail ? `<p style="font-size: 12px; color: #999;">Para dúvidas, entre em contato: ${adminEmail}</p>` : ''}
                  </div>
                </div>
              </body>
              </html>
          ...
          errorCount++
          errors.push(`Erro ao enviar para ${userEmail}: ${emailError.message}`)
          console.error(`Erro ao enviar email para ${userEmail}:`, emailError)
        }
      })

      // Aguardar todos os emails serem processados
      await Promise.all(emailPromises)

      console.log(`Email em massa enviado: ${successCount} sucesso, ${errorCount} erros`)
