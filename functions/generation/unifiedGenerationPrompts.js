...
function buildFlashcardPrompt(meta = {}, batchNumber, totalBatches, cardsInBatch, existingFronts = []) {
  ...
  const searchRule = isLikelyLegalDiscipline(meta.disciplina)
    ? 'OBRIGATÓRIO: use Google Search antes de citar leis/artigos nos versos. Se não confirmar, não cite número de lei.'
    : 'Use Google Search para fatos específicos ou datas quando necessário.'

  return `Gere flashcards para estudo de concurso.
...
LOTE: ${batchNumber}/${totalBatches} — exatamente ${cardsInBatch} cards neste lote.
${existingList}
...
{
  "flashcards": [
    { "frente": "pergunta objetiva", "verso": "resposta completa (2-4 frases)", "dificuldade": "fácil|médio|difícil" }
  ]
}

REGRAS:
- ${AI_TEXT_FORMAT_RULES}
- Versos completos e técnicos; nunca superficiais.
- Cobrir o tópico com profundidade; conteúdo fiel ao edital.
